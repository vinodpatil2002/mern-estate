  ...
  return (
    <div className="p-2 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <img onClick={()=>fileRef.current.click()} src={formData.avatar || currentUser.avatar} alt="profile"  className="rounded-full w-40 h-40 object-cover cursor-pointer self-center"/>
        <p className="text-sm self-center">{fileUploadError ? (<span className="text-red-700">Error image Uploaded (Image max size must be of 2MB) </span>)
        : filePer > 0 && filePer < 100 ? (<span className="text-slate-700"> 
        {`Uploading ${filePer}%`} </span>) : filePer===100 ? (<span className="text-green-700">Image Successfully Uploaded</span>) : ""
      }</p>
        ...
        <button disabled={loading} className="bg-slate-700 text-white p-3 uppercase rounded-lg hover:opacity-85 disabled:opacity-70">
